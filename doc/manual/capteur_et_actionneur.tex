...
correspond au "front panel", et plus précisément la partie MCP ajoutée au contrôle des trains d'atterrissage de
...
Le MCP du cockpit n'ayant pu être en notre possession à temps, nous avons dû nous contenter des informations
...
\subsubsection{Contexte et objectif}
Dans le MCP du cockpit, en plus des boutons et autres commutateurs on peut trouver plusieurs blocs d'afficheurs
...
Pour le test, nous avons utilisé un PCF dont 16 pattes ont été connectées à un niveau 1 récessif, ainsi qu'une à la masse simulant l'appui sur une touche. Les tests ont été concluant, montrant que le PcanView avait bien reçu le message CAN. Pour tester l'actualisation de l'affichage, nous avons utilisé le PcanView afin d'envoyer
un message CAN au BECK. La valeur affichée correspondant alors bien à la valeur contenue dans le message CAN, cet état a pu être validé.
...
est une interface adéquate pour un clavier (9+8=17 entrées/sorties). Du côté de BECK, il a été demandé de réaliser la 
...
Les registres de lecture et d'écriture sont illustrés sur la figure\ref{fig:SI2C_bloclink}. Un registre d'écriture 
