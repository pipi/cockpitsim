...
\begin{figure}[htpb]
  ...
  \caption{half_duplex}
  \label{fig:half_duplex_AX12}
\end{figure}

On souhaite donc, que les signaux, de niveau TTL, RxD et TxD dépendent du signal Direction_Port.
Lorsque le signal Direction_port est au niveau haut on ait le signal TxD la sortie qui représente DATA. Et lorsque, le signal Direction_port est au niveau bas que le signal DATA soit l’entrée du signale RxD.
On constate qu’on peut s’affranchir du CM5, d’une part à cause de son coût élevé (60€) et d’une autre part pour notre apprentissage, en réalisant une carte électrique qui produirait cette fonctionnalité du CM5.
