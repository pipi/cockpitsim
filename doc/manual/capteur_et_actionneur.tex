...
La programmation du GAlisp22v10, a était faite via Abel, cf.ANNEXE ?, afin de réaliser la fonction half_duplex, zexpliquée dans la partie précédente.
Les chronogrammes, cf.ANNEXE ?, montre bien que la fonction vérifie bien son rôle. On remarque que …..

	Pour la réalisation de la carte qui fait office d’interface entre le port rs232 et le servomoteur, nous avons optés pour un MAX233, car ce dernier exige moins de condensateurs ce qui facilite le routage par rapport au MAX232, qui lui en possède 5, tout en gardant les mêmes propriétés…

Pour l’alimentation de la carte, nous avons utilisé un régulateur 7805T,  capable de fournir une tension de 5v (output) en sortie afin d’alimenter le GAL et le MAX233 et d’avoir une tension de l’ordre de 9.6v (input) pour l’alimentation les AX12 :
...
\begin{figure}[htpb]
  ...
  \caption{7805}
  \label{fig:régulateur 7805_ carte AX12}
\end{figure}
