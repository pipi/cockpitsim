...
Pour débuter notre projet sur la réalisation du cockpit, nous avons eu besoin de positionner les différents éléments de notre travail dans "l'univers Flight Simulator". En effet, les données sont extraites différemment selon leurs types. Les données concernant l'affichage sur les écrans passent par les logiciels WideView. Les données concernant les paramètres réglables ou à communiquer à l'utilisateur par différents afficheurs (autre que celle simuler par les écrans) passent par le logiciel FSUIPC. Il a donc fallu réaliser une surcouche logiciel afin de pouvoir dialoguer avec ce dernier. 
...
Ce découpage a été déterminé selon les catégories d'instruments à réaliser. Les affichages par exemple ont été séparés selon leurs fonctions: ceux qui affichent le monde exterieure, sur lequel nous avions uniquement les paramètres de confguration des affichages à modifier pour gérer l'angle de vue des différents écrans par exemple ont fait l'objet d'une étude particulière par une sous-équipe du projet. Les écrans qui affichent les instruments de bord, distincts de l'affichage du monde extérieur ont fait l'objet d'une étude 
distincte également.

La partie qui concerne les commandes de gaz faisant appel à des servo-moteurs  intélligent permettant de le commander avec des données prétraitée comme une position ou un angle. En effet, c'est la seul partie qui utilise ce type de technologie.
...
nous avons choisi des solutions validées par nos professeurs. Nous nous sommes servi de l'état de l'art réalisé en amont pour définir nos choix. Cependant, les autres projets mettent souvent en en avant l'aspect visuel et se positionne du point de vue \emph{human in the
...
Nous avons utilisé deux modèles BECK différents : le sc123 et le sc 143 tous les deux de la famille sc1x3 donc très ressemblant. Le choix du BECK nous a parut évident car, tout d'abord, les encadrants de notre projet pouvaient nous en procurer rapidement, et surtout, parce que cette famille de contrôleur permettent facilement d'intégrer à la flois des réseaux CAN et I$^2$C, enfin la puissance de calcul suffisante et l'utilisation facile (code en C) a rendu le BECK très intéressant.

\subsubsection{Les missions des BECK}
Les BECK représentent la deuxième plus grande puissance de calcul du simulateur, de ce fait, ils auront de nombreuses missions à remplir :
\begin{itemize}
  \item correspondance messages CAN et message I$^2$C,
  \item décodage du clavier matriciel du MCDU
