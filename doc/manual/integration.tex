...
aussi bien le lien avec la partie matérielle que logicielle. En effet, il est question de lier les différentes briques logicielles
développées aux modules matériels produits.

Du côté logiciel, nous avons la partie dénomée FSAdapter, faisant l'interface avec Microsft Flight Simulator X, mais aussi
...
Avant d'expliquer la procédure d'intégration que nous avons suivie, il est nécessaire de faire un bref rappel du contexte. 

A ce stade du projet, nous disposons d'un module CAN générique fonctionnel prêt à se voir attribuer des actionneurs 
et des capteurs au travers d'un bus I$^2$C, ainsi que d'une liaison CAN active grâce à l'ajout dans FSAdapter d'une 
première famille \footnote{Les familles dans FSAdapter correspondent à des regroupements d'actionneurs et de capteurs 
dans la représentation du modèle de l'avion considéré. En l'occurrence, il peut s'agir de l'indicateur d'altitude du 
...
Cependant, malgré le fait que nous êtions prêts pour initier cette intégration, nous avons été confrontés à des barrières importantes.
En particulier, nous avons dû faire face à des retards dans les commandes qui nous ont empêché de pousser les tests préliminaires
suffisamment loin pour pouvoir mettre en oeuvre une intégration complète. C'est pourquoi, nous avons pris la partie de mettre en avant
...
Flight Simulator X et descendant jusqu'aux actionneurs connectés au module CAN via un bus I$^2$C.
...
le plus significativement possible les erreurs ou bogues et leurs causes, nous avons décidé de procéder par étapes. Cela peut paraître
...
De plus, nous avons décidé de partir des capteurs et actionneurs car ils offrent des données continues et complètements déterministes.
Ce dont nous n'étions pas sûr du côté de FSAdaper.
...
\begin{enumerate}
  ...
  \item Valider les données reçues par le module par rapport aux actions portées sur l'encodeur.
  ...
  \item Valider la mise à jour des données dans FSUIPC grâce au logiciel FSInterogate \footnote{FSInterogate donne accès aux
...
Pour effectué l'intégration, avant de lancer les procédures de tests, il nous fallait mettre en place la manipulation: préparer le
cablage, les alimentations, connecter les différents modules à leurs bus respectifs et finalement connecter le tout au PC central
via le bus CAN et le module USB/CAN.

Par ailleurs, cela à nécessité d'assigner une personne à temps plein sur cette partie. La tâche n'étant pas spécialement compliquée
mais demandant du temps et surtout beaucoup de rigueur afin d'éviter de créer des interférences lors des tests, leurs faisant perdre,
pour l'heure, leurs propriétés déterministes.
...
d'intégration. En effet, ces tests préliminaires ont élagué bon nombres de bogues et d'erreurs intrinsèques aux briques 
...
De plus, concernant ces formats de données, nous avons dû nous mettre d'accord sur l'ensemble du projet pour que tous les
...
Dès lors que les composants étaient prêts et une fois connectés, nous avons vu constater avec plaisir et satisfaction qu'ils envoyaient
les bonnes données et sous la forme escomptée.

Cette étape de l'intégration est certes courte mais néanmoins cruciale car les élements formant les actionneurs et les capteurs forment
...
Il a fallu effectuer beaucoup de modifications dans le code. En particulier, il était nécessaire de nettoyer le code qui portait
...
prise en compte des évolutions futures - sans pour autant les ignorer. Il fallait avant tout offrir une solution qui fonctionne,
...
Comme nous l'avons déjà souligné ci-dessus, notre objectif premier est avant tout d'offrir une solution fonctionnelle pour effectuer, avant toute autre chose, la preuve de notre concept. De ce fait, les choix d'implantation de la solution seront amenés à changer
fortement par la suite. En particulier la gestion des données reçues via le bus CAN.

Maintenant que nous avons posé le contexte de l'intégration de cette partie du projet, nous pouvons maintenant passer aux commentaires
sur les résultats de cette dernière.

En premier lieu, nous avons été rapidement confrontés à des problèmes de représentation de données. Bien qu'une convention avait été
...
La suite de l'intégration nous a pourtant demandé de revoir notre modèle de messages transitant sur le bus CAN. En effet, il fallait
...
N'oubliant pas, par ailleurs, que nous voulons avant tout faire la preuve de notre concept, nous avons décidé de faire un choix
