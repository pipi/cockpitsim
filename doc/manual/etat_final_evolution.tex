...
Jusqu'à présent, tous les éléments  interfacés sur le bus I2C, à savoir switchs, commutateurs à positions, encodeurs incrémentaux, ...  sont systématiquement interrogés par FSAdapter via les contrôleurs BECK. Cette solution n'est vraisemblablement pas la meilleure. En effet, les interrogations de manière continue nécessitent une énorme charge de travail sur  FSAdapter ou tout du moins une forte utilisation du le bus CAN. Ce qui est d'autant plus vrai qu'il y a beaucoup d'éléments à interroger.
...
Par conséquent, cela nécessite de repenser le bloc I2C du FPGA, dans le cas où ce type de structure est capable de générer des interruptions. Il faudrait donc créer un module pouvant devenir 'maître' et 'esclave' sur le bus I2C afin qu'il puisse prendre le contrôle du bus et générer une trame I2C en destination de FSAdapter et se déconnecter du bus lorsqu'une communication en cours ne le concerne pas. Cela impliquerait alors de placer le contrôleur BECK en 'esclave' I2C. 
Si le FPGA n'était pas en mesure de générer ce type d'évènement, la piste du microcontrôleur pourrait éventuellement être réouverte.
...
deuxième solution existe consistant à utiliser le FPGA. Dans ce cas il faudra implanter un routage qui permettrait
...
La communication Beck-Ax12 fonctionnant correctement et le code pour la communication Beck-Can étant implémenté mais non testé, la prochaine étape pourrait être le test de cette deuxième partie.
Il faudra penser à initialiser les servomoteurs, réalisant le deuxième moteur et le trim, à un débit de 75600 bauds via le MC5 afin que les servomoteurs puissent communiquer avec le port série.
