...
Jusqu'à présent, tous les éléments  interfacés sur le bus I2C, à savoir switchs, commutateurs à positions, encodeurs incrémentaux, ...  sont systématiquement interrogés par FSAdapter via les microcontrôleurs BECK. Cette solution n'est vraisemblablement pas la meilleure. En effet, mes interrogations de manière continue nécessitent une énorme charge à  FSAdapter ou tout du moins intensité le trafic sur le bus CAN. Ce qui est d'autant plus vrai qu'il y a d'élément à interroger.

La majorité des requêtes envoyées par FSAdapter ne sont effectivement pas utiles puisqu'aucun élément n'a changé de valeur.  L'amélioration qui peut donc être envisagé est de ne plus demander à FSAdatper de faire interrogations continues vers les éléments I2C mais de faire en sorte que les composants extérieurs puissent envoyer un message dès qu'un évènement s'est passé (i.e. un commutateur à changé de position, il doit générer un message vers FSAdapter sans aucune interrogation de ce dernier). Ainsi tous les éléments extérieurs doivent être capables de générer des interruptions pour avertir d'un changement et prendre en compte des conflits en particulier sur le bus pouvant survenir si jamais plusieurs éléments sont manipulés en même temps. 

Par conséquent, cela nécessite de repenser le bloc I2C du FPGA, dans le cas où ce type de structure est capable de générer des interruptions. Dans le cas où cela serait possible, il faudrait donc intégrer ou les remplacer par un maître I2C afin qu'il puisse prendre le contrôle du bus et générer une trame I2C en destination de FSAdapter. Si le FPGA n'était pas en mesure de générer ce type d'évènement, la piste sur le microcontrôleur pourrait éventuellement être réouverte.
