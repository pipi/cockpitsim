...
\section{histoire d'évolution} 
Tout d'abord, étant donné que nous ne connaissions pas toutes les variables que nous seront amenés à utiliser, nous avons décidé de créer un logiciel pouvant ajouter n'iporte quelle valeur provenant de FSUIPC, sans modifier directement le programme. Les données provenant de FSUIPC étaient sauvgardées dans un fichier texte pouvant être lu par le programme.

 Toutes les données sont fournit par le FSUIPC et comme ils sont tous dans le tableau,nous pouvons donc modifier le donné que nous avons besoin sans accÈder directement dans le FSUIPC. Et nous avons pensÈ que mÍme si nous ne pouvons pas utiliser le fichier cette fois, cela sera peut Ítre utile pour les gens suivant.
Lors de l'étude initiale sur cette partie du projet, on a fait le choix d'écrire le code en C++. Par la suite, et sur les conseils de notre tuteur, nous sommes passé à une solution en C. Cependant, nous nous sommes rapidement aperçus que cette solution engendrerait beaucoup de possibilités d'erreurs et de bogues pour des raisons que nous n'aborderons pas ici et le programme devient aussi très compliqué. Il a donc été fait le choix de revenir à la solution initiale, à savoir écrire notre logiciel en C++.

\section{Cahier des charges du logiciel}
Comme nous l'avons précisé précédemment, l'objectif de cette partie est de relier Flight Simulator X à notre 
cockpit. Plus précisément, nous voulions créer une variable pour chaque boutons du cockpit (par exemple, 
...
(de CAN vers FS) les trois derniers bits permettent de déterminer quel paramètre de la trame à été modifiée. Les autres bits de 
...
Cette partie du programme lit les messages provenant du bus CAN et après traitement de celles-ci, elle met à jour les données sur FSUIPC.
...
poids fort de l'identifiant. Les 3 bits de poids faibles permettent de connaître la variable dans une trame donnée qui a été modifiée. Le logiciel 
...
structure \textit{from-can}. La table \textit{to-can} est mise à jour tout les 50ms. Lorsqu'une valeur est modifiée, une fonction de traitement est 
appelée. Cette fonction prend la valeur provenant de FSUIPC et calcule la donnée à envoyer à tracers le bus CAN. Dès qu'une donnée a été 
