...
maximales. Pour différentier les trames, on y associe un identifiant sur 11 bits qui est défini de la manière suivante. 
Lorsque le message va vers le simulateur (FS -> CAN) les trois derniers bits de l'identifiant sont à 0. Dans le sens inverse,
(CAN -> FS) les trois dernier bits permettent de déterminer quel paramètre de la trame à été modifié. Les autres bits de 
l'identifiant permettent de différentier les différentes familles de trames et les trames dans une même famille (par exemple,
famille A, on aura les identifiants 100,108,110, pour la famille B, on aura 200,208). Unefamille est le regroupement de plusieurs 
fonctionnalitées fesant appel à un même type de materiel (moteur, front de panel, manettes des gas...).
...
\begin{figure}[htpb]
  ...
  \caption{modèle d'écriture vers le bus CAN}
  \label{fig:write}
\end{figure}
...
\begin{figure}[htpb]
  ...
  \caption{modèle de lecture du bus CAN}
  \label{fig:Read}
\end{figure}